        ...
        if (info.result.code == 0) {
          info.data.forEach((item, index) => {
            if (item.Createdate == CreateDate) {
              ...
              let CreateDate = utils.formatDate.formatTime(utils.formatDate.getDateStringTime(item.Createdate));
              parmsObj = { condition: VehicleID + '@' + CreateBy + '@' + CreateDate };
              deleteremark(parmsObj).then(function (res) {
                let info = res.data;
                if (info.result.code == 0) {
                  _this.$message.success('备注删除成功！');
                  _this.readNoteInformation();
                }
              });
            }
          })
