...
<template>
  <!-- <div>  -->
      <div :id="mapid">
         <MonitorData v-show="false" ref="childMethod"></MonitorData>
      </div>
      <!-- 在父组件调用子组件的方法 -->
...
import Tree from '@/components/mapTool/tree';
import MonitorData from '@/components/table/monitorList'// 监控列表
import { monitorlsitDetailsAxios } from '@/Api/mapApi.js';
export default {
  props: ['mapid'],
  components: {
    MonitorData
  ...
  methods: {
    createAbnormalMarker (row, index) {
      ...
      cImg.src = 'static/images/car2.png'
    ...
    InitMap () {
      ...
      if (this.map === null && this.mapid === 'mapContainer') {
        ...
        map.plugin(['AMap.ToolBar'], function () {
          ...
          var tool = new AMap.ToolBar({
            offset: new AMap.Pixel(35 / 0.8, 100 / 0.8),
            position: 'RT'
            // zoom: 0.7
          });
          map.addControl(tool);
      ...
      if (!this.$hisMapArr) {
        this.createHisMap(theme)
      } else {
        let index = this.$hisMapArr.IndexOf(this.mapid, d => Object.keys(d)[0])
        if (index !== -1) {
          this.map = Object.values(this.$hisMapArr[index])[0]
        } else {
          this.createHisMap(theme)
        }
      }
      // if (!this.$hisMapArr && this.mapid.indexOf('maphis') != -1) {
      //   var map = new AMap.Map(this.mapid, {
      //     resizeEnable: true
      //   });
      //   // 主题控制 如果satellite卫星图 默认加载卫星图
      ...
      //   // var hisMapArr = this.$hisMapArr
      //   var hismap = {};
      //   hismap[this.mapid] = map;

      //   Vue.prototype.$hisMapArr = Vue.prototype.$hisMapArr || [] // 申明全局默认历史轨迹地图实例
      //   // hisMapArr.push(hismap);
      //   this.$hisMapArr.push(hismap);
      // } else {
          ...
          map.plugin(['AMap.MouseTool'], function () {
            ...
            that.MousetoolSign.on('draw', function (even) {
              if (that.signMarkerInfoWindow === null) {
                that.signMarkerInfoWindow = new AMap.InfoWindow({
              ...
              marker.on('mouseover', function (e) {
                ...
                var content =
                  '<div class="pointwind">\
                              <input class="inputbz" placeholder="标注名称" value="' +
                  data.name +
                  '"/>\
                              <textarea class="inputbz" placeholder="备注">' +
                  data.remark +
                  '</textarea>\
                            </div>';
                // 获取当前标记点
              ...
              $(document).on('change', '.inputbz', function (e) {
                ...
                if ($(e.target).is('input')) {
                  ...
                  formdata.remark = $(e.target)
                    .next()
                    .val();
                } else {
                  formdata.name = $(e.target)
                    .prev()
                    .val();
    ...
    createMarkerCar (data) {
      ...
      let lat = null;
      if (!realdata) return;
      // (lng = realdata.lng || 0), (lat = realdata.lat || 0);

      [lng, lat] = utils.transformWGStoGCJ(realdata.lng, realdata.lat)

      // 实例化车辆marker
      marker = new AMap.Marker({
        map: this.map,
        title: basicdata.carno + ' ' + basicdata.ownername,
        angle: realdata.direct - 90,
        autoRotation: true,
        topWhenClick: true,
        offset: new AMap.Pixel(-22.5, -23), // 这里就是
        position: [lng, lat]
      });
      console.log('bug---', realdata.direct);
      // 创建车辆图标
      var cImg = document.createElement('img');
      cImg.className = 'cIcon';
      ...
      marker.on('mouseover', function (e) {
        ...
        _this.$store.dispatch('initMoveCarInfo')
        let monitorData = _this.$store.getters.monitorData
        let index = monitorData.IndexOf(data.basicdata.vehicleid, d => d.vehicleid)
        let carInfo = monitorData[index]
        // data.lng = carInfo.realdata.lng
        // data.lat = carInfo.realdata.lat
        // _this.$store.commit('updateCarInfo', data);
        _this.$store.commit('updateCarInfo', carInfo);
        _this.$store.commit('setSelectVhID', data.vehicleid)
      ...
      this.$store.commit('add_carMarker', {
        marker: marker,
        markerLabel: marker_label
      });
    },
    createFortificationMarker (data) {
      // let _this = this
      let marker = null
      // let marker_label = null
      let [lng, lat] = [data.lg, data.lt]
      // let [lng, lat] = utils.transformWGStoGCJ(+data.lg, +data.lt)
      ...
      this.$store.commit('add_ydsfMarker', {key: data.vehicleid, marker})
    },
    // 创建自定义窗体
    createInfoWindow () {
      function getPositionArr (length, width = 26) {
        // let oWidth = width * 5
        let oWidth = 90 // width * 5
        let oHeight = oWidth
        let R = oWidth / 2
        let Ra = 360 / length
        let arr = []
        for (let i = 0; i < length; i++) {
          ...
          let _y = 0
          let _ra = 90 - Ra * i
          let coordX = R * Math.cos(_ra / 180 * Math.PI)
          let coordY = R * Math.sin(_ra / 180 * Math.PI)
          let pX = R + coordX
          let pY = R - coordY
          let absX = pX - _x - 13
          let absY = pY - _y + 13
          let abxArr = [absX, absY]
          arr.push(abxArr)
        }
        return arr
      }
      ...
      // 创建 功能按钮
      let obj = [
        {'name': 'ydsf', icon: 'iconfont icon-anquan', title: '原地设防'},
        {'name': 'lsgj', icon: 'iconfont icon-guijihuifang', title: '历史轨迹'},
        // {'name': 'zhiling', icon: 'iconfont icon-qizi_icon', title: '指令'},
        {'name': 'follow', icon: 'iconfont icon-guanzhu', title: '关注', fid: ['1', '2', '3', '4', '5']},
        {'name': 'dzwl', icon: 'iconfont icon-dianziweilan', title: '电子围栏'}
        // {'name': 'shuaxin', icon: 'iconfont icon-shuaxin', title: '刷新'}
      ]
      let arrFilter = obj.filter(d => {
        switch (d.name) {
          case 'follow':
      ...
      let positionArr = getPositionArr(arrFilter.length)
      arrFilter.forEach((cell, i) => {
        cell.dom = document.createElement('span')
        cell.dom.className = cell.icon
        cell.dom.title = cell.title
        cell.dom.style.top = positionArr[i][1] + 'px'
        cell.dom.style.left = positionArr[i][0] + 'px'
        cell.dom.name = cell.name

        switch (cell.name) {
          case 'ydsf':
            break;
          case 'shuaxin':
            cell.dom.onclick = _this.refreshBox
            break

          default:
            break;
        }
        divul.appendChild(cell.dom)
      })
      $(document)
        .off('click', '.markerInfo span.iconfont')
        .on('click', '.markerInfo span.iconfont', this.handType);
    ...
    refreshBox () {
      ...
      var _this = this;
      $('.info-top span.shuaxin').addClass('cur');
      this.$indexedDB.getDataById(
        'vehicleDetail',
        _this.vehInfo.vehicleid,
        function (result) {
          if (result.error === 0) {
            var data = result.data;
            let old = _this.$refs.childMethod.monitorData
            let currentIndex = old.IndexOf(data.vehicleid, d => d.basicdata.vehicleid)
            data.realdata = old[currentIndex].address || old[currentIndex].realdata.address
            ...
            setInterval(function () {
              ...
              $('.info-top span.shuaxin').removeClass('cur');
            }, 1000);
          }
        }
      );
    ...
    mapThemeRefresh (enName) {
      if (enName === 'satellite') {
        ...
        this.$hisMapArr && this.$hisMapArr.forEach(obj => {
          Object.values(obj)[0].add([
            new AMap.TileLayer.Satellite(),
            new AMap.TileLayer.RoadNet()
          ])
        })
        ...
        this.$hisMapArr && this.$hisMapArr.forEach(obj => {
          Object.values(obj)[0].setLayers([new AMap.TileLayer()]);
          Object.values(obj)[0].setMapStyle('amap://styles/' + enName);
        })
      }
    },
    // 车辆弹框事件调用
    ...
    dblclick () {
      ...
      dashed.ondblclick = function () {
        var vehicleid = _this.vehInfo.vehicleid;
        _this.$refs.childMethod.pushDetails(vehicleid)
      };
    }
...
<style>
#VPS #mapContainer .amap-controls .amap-toolbar{
  zoom: .8;
}
</style>
